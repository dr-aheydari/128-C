...
%% part Fivr: Doubling the damping coefficient and checking the for critical
%   wind speed 
...
if (a == 1)
   y_t = tacoma([0 1000],[1 0 0.001 0],25000,5);
...
plot(t_step, y_t(:,1));
xlabel('Time');
ylabel('y(t) vertical disturbances');
title(' Vertical Changes When Damping Coefficent is Doubled ');
hold off
figure(3)
hold on 
plot(t_step, y_t(:,2));
...
title(' Angular Changes When Damping Coefficent is Doubled  ');
...
ydots(1) = y(2);
ydots(2) = -0.02 * y(2) - 0.4 * (a1 + a2 - 2) / a + 0.2 * W * ...
    sin(omega * t);
ydots(3) = y(4);
